...
wx_time_per_block = wx_per_block.map(lambda (block, list_of_sigmets): (block, sum([u.time_string_to_timestamp(s.properties.validTimeTo) -  u.time_string_to_timestamp(s.properties.validTimeFrom) for s in list_of_sigmets])))
