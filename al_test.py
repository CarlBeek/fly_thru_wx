...
total_ac = airlines_per_block.count()
...
ac_probs_counts = ac_count.map(lambda (block, count): (block, (count, count/float(total_ac))))

#ac_counts.toDF().write.json("file:///home/s1542648/ac_count.json", mode="overwrite")
ac_probs_counts.toDF().write.json("hdfs:///user/s1638696/ac_output/ac_count_probs.json", mode="overwrite")
